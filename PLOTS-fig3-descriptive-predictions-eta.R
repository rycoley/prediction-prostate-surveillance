...
bx_data<-read.csv("simulation-data/bx-data-sim.csv")
...
etahat<-read.csv(paste("results/jags-prediction-unadj-eta_hat-1.csv",sep=""))
dim(etahat)
etahat<-as.matrix(etahat[,2:dim(etahat)[2]])
for(i in 2:5){
	res<-read.csv(paste("results/jags-prediction-unadj-eta_hat-",i,".csv",sep=""))
...
pdf("plots/figure3a-histogram.pdf", width=9, height=7)
...
pdf("plots/figure3b-scatter-eta-iop-vs-unadj.pdf", width=9, height=7)
par(mar=c(5,5.5,2,5))
