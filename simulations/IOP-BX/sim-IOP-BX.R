...
### This code is used to run a single simulation for the jags model that accounts for a possible biopsy informative observation process (IOP).
...
# (3) Shape data in prep for JAGS run
#use code from IOP folder here
source("simulations/IOP/IOP-prep-data-for-jags.R")

# (4) Run JAGS
source("simulations/IOP-BX/IOP-BX-call-jags.R")

# (5) Save posterior samples
for(j in 1:length(out$sims.list)){
	if(!names(out$sims.list)[j]=="eta_hat"){
	write.csv(out$sims.list[[j]], paste("simulations/IOP-BX/results/jags-prediction-IOP-BX-", names(out$sims.list)[j],"-",SEED,".csv",sep=""))}}

##Note- These results are not saved in the github folder. (It is too cumbersome). Use sim-IOP-BX-res.R, sim-IOP-BX-res-eta.R, and IS-for-eta-known-IOP-BX.R to obtain summaries of posterior results and predictions 
...
write.csv(eta_to_save, paste0("simulations/IOP-BX/results/eta-true-vs-pred-IOP-BX-",SEED,".csv"))
