...
for(p in 1:length(params)){
	res<-NA
	res<-read.csv(paste0("simulations/IOP-BX/results/jags-prediction-IOP-BX-", params[p], "-", SEED, ".csv"))
	D<-dim(res)[2]
...
if(sum(subj_rc==i)>0){
	lik <- exp(log_lik_psa + log_lik_bx + log_lik_rc )}else{lik <- exp(log_lik_psa + log_lik_bx )}
...
write.csv(eta_to_save, paste0("simulations/IOP-BX/results/eta-known-true-vs-pred-IOP-BX-",SEED,".csv"))
...
save(auc.res=auc.res, mse.res=mse.res, mae.res=mae.res, file="simulations/IOP-BX/results/sim-IOP-BX-res-eta-known.RData")
