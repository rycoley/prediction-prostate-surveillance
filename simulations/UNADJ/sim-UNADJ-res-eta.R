...
### This code is used to summarize predictive accuracy of true state predictions from the unadjusted (non-IOP) model
...
for(j in 1:nsims){
	res<-read.csv(paste0("simulations/UNADJ/results/eta-true-vs-pred-unadj-",j,".csv"))
...
save(auc.res=auc.res, mse.res=mse.res, mae.res=mae.res, file="simulations/UNADJ/results/sim-UNADJ-res-eta.RData")
