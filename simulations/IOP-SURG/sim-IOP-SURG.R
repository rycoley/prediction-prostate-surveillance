...
### This code is used to run a single simulation for the jags model that accounts for a possible surgery informative observation process (IOP).
...
# (3) Shape data in prep for JAGS run
#this code is in the IOP folder
source("simulations/IOP/IOP-prep-data-for-jags.R")

# (4) Run JAGS
source("simulations/IOP-SURG/IOP-SURG-call-jags.R")

# (5) Save posterior samples
for(j in 1:length(out$sims.list)){
	if(!names(out$sims.list)[j]=="eta_hat"){
	write.csv(out$sims.list[[j]], paste("simulations/IOP-SURG/results/jags-prediction-IOP-SURG-", names(out$sims.list)[j],"-",SEED,".csv",sep=""))}}

##Note- These results are not saved in the github folder. (It is too cumbersome). Use sim-IOP-SURG-res.R, sim-IOP-SURG-res-eta.R, and IS-for-eta-known-IOP-SURG.R to obtain summaries of posterior results and predictions
...
write.csv(eta_to_save, paste0("simulations/IOP-SURG/results/eta-true-vs-pred-IOP-SURG-",SEED,".csv"))
