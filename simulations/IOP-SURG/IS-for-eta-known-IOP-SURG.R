...
for(p in 1:length(params)){
	res<-NA
	res<-read.csv(paste0("simulations/IOP-SURG/results/jags-prediction-IOP-SURG-", params[p], "-", SEED, ".csv"))
	D<-dim(res)[2]
...
if(sum(subj_rc==i)>0){
	lik <- exp(log_lik_psa + log_lik_rc + log_lik_surg)}else{lik <- exp(log_lik_psa +  log_lik_surg)}
...
write.csv(eta_to_save, paste0("simulations/IOP-SURG/results/eta-known-true-vs-pred-IOP-SURG-",SEED,".csv"))
...
save(auc.res=auc.res, mse.res=mse.res, mae.res=mae.res, file="simulations/IOP-SURG/results/sim-IOP-SURG-res-eta-known.RData")
