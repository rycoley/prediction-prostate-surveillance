...
### This code is used to pull in data for individuals, MCMC results of trajectories, and make individual-level plots, as seen in appendix figure 22
...
load("jhas-analysis/simulation-data/pars-for-data-sim.RData")
...
### GET DATA
source("jhas-analysis/IOP/IOP-prep-data-for-jags.R")
...
b_vec<-read.csv(paste("jhas-analysis/IOP/results/jags-prediction-iop-b_vec-1-reduced.csv",sep=""))
b_int<-as.matrix(b_vec[,(pred_ids)+1])
b_slope<-as.matrix(b_vec[,(n+1+pred_ids)])
for(i in 2:5){
	res<-read.csv(paste("jhas-analysis/IOP/results/jags-prediction-iop-b_vec-",i,"-reduced.csv",sep=""))
...
pdf("jhas-analysis/plots/appendix-figure22-individual-data-and-predictions.pdf", height=10, width=8)
