...
### This code is used to call the UNADJUSTED JAGS model, that is, it does not include biopsy and surgery informative observation process (IOP) components
...
source("jhas-analysis/UNADJ/UNADJ-prep-data-for-jags.R")
...
do_one<-function(seed){
	set.seed(seed)	
	outj<-jags(jags_data, inits=inits, parameters.to.save=params, model.file="jhas-analysis/UNADJ/UNADJ-jags-model.txt", n.thin=nt, n.chains=nc, n.burnin=nb, n.iter=ni)

out<-outj$BUGSoutput

for(j in 1:length(out$sims.list)){
	write.csv(out$sims.list[[j]], paste("jhas-analysis/UNADJ/results/jags-prediction-unadj-", names(out$sims.list)[j],"-",seed,".csv",sep=""))}
}
