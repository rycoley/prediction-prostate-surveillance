...
# parameters to track
params <- c("p_eta", "eta_hat", "mu_int", "mu_slope", "sigma_int", "sigma_slope", "sigma_res", "rho_int_slope", "cov_int_slope", "b_vec", "beta", "nu_BX", "gamma_RC", "omega_SURG", "p_bx", "p_rc", "p_surg")  
...
do_one<-function(seed){
	set.seed(seed)	
	outj<-jags(jags_data, inits=inits, parameters.to.save=params, model.file="jhas-analysis/IOP/IOP-jags-model.txt", n.thin=nt, n.chains=nc, n.burnin=nb, n.iter=ni)

out<-outj$BUGSoutput

for(j in 1:length(out$sims.list)){
	write.csv(out$sims.list[[j]], paste("jhas-analysis/IOP/results/jags-prediction-iop-", names(out$sims.list)[j],"-",seed,".csv",sep=""))}
}
