...
do_one<-function(seed){
	set.seed(seed)	
	outj<-jags(jags_data, inits=inits, parameters.to.save=params, model.file="jhas-analysis/IOP/IOP-SENSITIVITY-ANALYSIS-jags-model.txt", n.thin=nt, n.chains=nc, n.burnin=nb, n.iter=ni)

out<-outj$BUGSoutput

for(j in 1:length(out$sims.list)){
	write.csv(out$sims.list[[j]], paste0("jhas-analysis/IOP/results/jags-prediction-iop-sa-", prior_coef[1], "-", prior_coef[2], "-", names(out$sims.list)[j],"-",seed,".csv"))}
}
