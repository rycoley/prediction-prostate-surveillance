...
res<-read.csv(paste("results/jags-prediction-iop-nu_BX-1.csv",sep=""))
#dim(res)
...
res<-read.csv(paste0("results/jags-prediction-iop-sa-",mu.a[j],"-",mu.b[k],"-nu_BX-1.csv"))
nu_bx<-res[,dim(res)[2]]
for(i in c(2:5)){
	res<-read.csv(paste0("results/jags-prediction-iop-sa-",mu.a[j],"-",mu.b[k],"-nu_BX-",i,".csv"))
	nu_bx<-c(nu_bx,res[,dim(res)[2]])}
#i<-5
...
res<-read.csv(paste("results/jags-prediction-iop-omega_SURG-1.csv",sep=""))
#dim(res)
...
for(j in 1:3){for(k in 1:3){
res<-read.csv(paste0("results/jags-prediction-iop-sa-",mu.a[j],"-",mu.b[k],"-omega_SURG-1.csv"))
omega_surg<-as.matrix(res[,(dim(res)[2]-1):dim(res)[2]])
for(i in c(2:5)){
	res<-read.csv(paste0("results/jags-prediction-iop-sa-",mu.a[j],"-",mu.b[k],"-omega_SURG-",i,".csv"))
	omega_surg<-rbind(omega_surg,res[,(dim(res)[2]-1):dim(res)[2]])}
#i<-5
...
pdf("plots/figure7-posterior-coef-eta.pdf", width=18, height=8)
...
for(j in 1:3){for(k in 1:3){
		lines(density(nu_bx_main[,(3*(j-1)+1+k)]), lwd=2, lty=c("dotted", "dotdash", "dashed")[j], col=c("gray30", "gray50", "gray70")[k])}}
...
for(j in 1:3){for(k in 1:3){
		lines(density(omega_surg_main[,(3*(j-1)+1+k)]), lwd=2, lty=c("dotted", "dotdash", "dashed")[j], col=c("gray30", "gray50", "gray70")[k])}}
...
for(j in 1:3){for(k in 1:3){
		lines(density(omega_surg_int[,(3*(j-1)+1+k)]), lwd=2, lty=c("dotted", "dotdash", "dashed")[j], col=c("gray30", "gray50", "gray70")[k])}}
...
legend(x=0.65, y=0.9, title="Prior Mean OR for Interaction on Surgery", legend=seq(-1,1,1), col=c("gray30","gray50","gray70"), lwd=rep(2,3), bty="n",horiz=T, cex=2)
